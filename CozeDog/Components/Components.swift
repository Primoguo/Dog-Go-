...
struct PixelDogSprite: View {
    ...
    var body: some View {
        ZStack {
            ...
            PixelRect(color: Color(hex: appearance.primaryFurHex))
            ...
            PixelRect(color: Color(hex: earColor))
            ...
            PixelRect(color: Color(hex: legColor))
            ...
            if breed == .shiba {
                ...
                PixelRect(color: Color(hex: tailColor))
    ...
    private var headWidth: CGFloat {
        breed == .golden ? 0.44 : 0.40
    }

    private var headHeight: CGFloat {
        breed == .golden ? 0.32 : 0.36
    }
    ...
    private var earWidth: CGFloat {
        breed == .golden ? 0.16 : 0.13
    }

    private var earHeight: CGFloat {
        breed == .golden ? 0.22 : 0.19
    }

    private var earY: CGFloat {
        breed == .golden ? -0.28 : -0.35
    }
    ...
    private var tailColor: UInt {
        appearance.marking == .tailTip ? appearance.secondaryFurHex : appearance.primaryFurHex
    }

    private var legColor: UInt {
        appearance.marking == .paws ? appearance.secondaryFurHex : appearance.primaryFurHex
    }

    private var earColor: UInt {
        if breed == .borderCollie { return appearance.secondaryFurHex }
        return appearance.marking == .ears ? appearance.secondaryFurHex : appearance.primaryFurHex
    }
...
struct DogChoiceCard: View {
    ...
    private var faceForeground: Color {
        dog == .borderCollie || dog == .native ? .white : Color(hex: 0x3C2715)
    }
}
